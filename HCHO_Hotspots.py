...
# Data Load 
df_raw = pd.read_csv('ui_ready_data.csv')
...
maharashtra_districts = {
    ...
    'Buldhana': (20.5033, 75.2236), 'Buldana': (20.5033, 75.2236), 'Latur': (18.4088, 76.3864),
    'Osmanabad': (17.8601, 76.7504),'Solapur': (17.6869, 75.9255),
    ...
    'Gondiya': (21.4598, 80.1950),  'Nanded': (19.1628, 77.3176)
}
...
# Aggregate by district (monthly mean / any hotspot)
df_agg = df_month.groupby('District').agg(
    HCHO=('HCHO', 'mean'),
    frp=('frp', 'mean'),
    Wind_Speed_kmh=('Wind_Speed_kmh', 'mean'),
    Compass_Dir=('Compass_Dir', lambda x: x.dropna().mode().iloc[0] if not x.dropna().empty else 'N/A'),
    Is_Hotspot=('Is_Hotspot', 'any'),
).reset_index()
...
with map_col:
    ...
    for _, row in df_agg.iterrows():
        ...
        wind_dir = row['Compass_Dir']
        wind_speed = row['Wind_Speed_kmh']

        is_hotspot = row.get('Is_Hotspot', False)
        hotspot_tag = '<span style="background:#ef4444;color:white;padding:2px 8px;border-radius:999px;font-size:0.72rem;font-weight:600;margin-left:5px;">🔥 HOTSPOT</span>' if is_hotspot else ""
        ...
        <div style="font-family:Inter,Arial;width:260px;padding:4px;">
          ...
          <span style="background:{color};color:white;padding:2px 8px;border-radius:999px;font-size:0.72rem;font-weight:600;">{status}</span>{hotspot_tag}
          <hr style="margin:8px 0;border-color:#e2e8f0;">
          <b>{main_stat}</b><br>
          🔥 Fire Power: {f"{row['frp']:.2f}" if not pd.isna(row['frp']) else "N/A"}<br><br>
          💨 Wind: {wind_dir if wind_dir and wind_dir != 'N/A' else 'N/A'} ({f"{wind_speed:.1f} km/h" if not pd.isna(wind_speed) else "N/A"})<br>
        ...
        if is_hotspot:
            folium.Marker(
                ...
                popup=folium.Popup(popup_html, max_width=300),
                tooltip=f"🔥 Hotspot Detected in {district}"
            ).add_to(m)
...
with legend_col:
    ...
    <div class="legend-box">
      ...
      <div class="legend-item">
        <div style="font-size:1.15rem;margin-top:-4px;margin-right:2px;">🔥</div>
        <div><b>Hotspot Detected</b><br><small>HCHO hotspot identified in dataset</small></div>
      </div>
      <hr style="border-color:#334155;margin:14px 0;">
      <h4>🌡️ Heatmap</h4>
      ...
      <p style="font-size:0.78rem;color:#94a3b8;line-height:1.5;">
        ...
        <code style="background:#0f172a;padding:2px 6px;border-radius:4px;font-size:0.75rem;">cigs = HCHO_Index / 23</code>
      </p>
    </div>
    """, unsafe_allow_html=True)
...
if search_query:
    ...
    if pd.isna(wind_dir) or wind_dir == 'N/A':
        wind_display = "N/A"
    else:
        wind_display = f"{wind_dir} ({wind_speed:.1f} km/h)"
    ...
    is_hotspot_val = row.get('Is_Hotspot', False)
    hotspot_badge = '<span class="tag" style="background:rgba(239,68,68,0.2);color:#ef4444;border:1px solid #ef4444;margin-left:10px;">🔥 HOTSPOT DETECTED</span>' if is_hotspot_val else ""

    st.markdown(f"""
<div class="district-card">
<h2>{search_query}</h2>
<span class="tag" style="background:{bg};color:{tag_color};border:1px solid {tag_color}40;">● {status}</span>{hotspot_badge}
...
<div class="info-item">
<div class="ilabel">Hotspot Status</div>
<div class="ivalue" style="color:{'#ef4444' if is_hotspot_val else '#22c55e'};">{'🔥 Hotspot Active' if is_hotspot_val else '✔️ Normal'}</div>
</div>
<div class="info-item">
...
<div class="info-item">
<div class="ilabel">Wind Info</div>
<div class="ivalue">💨 {wind_display}</div>
</div>
<div class="info-item">
